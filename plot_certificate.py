...
def main():
    ...
    if joint:
        if joint[0] == 'adj':
            colors = ["#ffffff", "#8cc2dd", "#8db1cd", "#235991"]
        elif joint[0] == 'att':
            colors = ["#ffffff", "#c2c0e7", "#704da8", "#453d7c"] 
        else:
            raise ValueError("joint certificate slice must be either 'adj' or 'att'")
    elif singular == 'adj':
        colors = ["#ffffff", "#c2c0e7", "#704da8", "#453d7c"] 
        ...
        for j in range(heatmap.shape[1]):
            if heatmap[i, j] >= level:
                if heatmap[i+1, j] < level:
                    plt.step([j, j+1], [i+1, i+1], where='mid', color=color, linestyle='--')
                if heatmap[i, j+1] < level:
                    plt.step([j+1, j+1], [i, i+1], where='mid', color=color, linestyle='--')
    ...
    for i in range(heatmap.shape[0]):
        for j in range(heatmap.shape[1]):
            if heatmap[i, j] >= level:
                if heatmap[i+1, j] < level:
                    plt.step([j, j+1], [i+1, i+1], where='mid', color=color)
                if heatmap[i, j+1] < level:
                    plt.step([j+1, j+1], [i, i+1], where='mid', color=color)

    from matplotlib.lines import Line2D
    legend_elements = [
        Line2D([0], [0], color=color, linestyle='--', lw=2, label='0.2 Contour'),
        Line2D([0], [0], color=color, lw=2, label='0.4 Contour')
    ]
