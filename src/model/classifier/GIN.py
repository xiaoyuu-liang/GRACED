...
class GIN(nn.Module):
    def __init__(self, n_feat, n_class, n_layer, agg_hidden, fc_hidden, dropout, readout, device, neighbor_pooling_type = 'sum', graph_pooling_type = 'sum', learn_eps = True):
        ...
        self.batch_norms = torch.nn.ModuleList([nn.BatchNorm1d(agg_hidden) for _ in range(n_layer)])
    ...
    def __get_edge_adj_matrix(self, adj_list):
        ...
        for adj in adj_list:
            ...
            if G.number_of_edges() == 0:
                ...
                edge_adj_list.append(torch.LongTensor([]).reshape(2, 0))
                continue
    ...
    def forward(self, data):
        ...
        # If x_list is a tensor
        h = h.reshape(len(x_list), -1, h.size()[-1])
        ...
        h = F.relu(self.fc1(h))
        h = self.fc2(h)  # Keep the logits without softmax if using CrossEntropyLoss
