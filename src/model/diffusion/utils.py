...
def get_one_hot(attr):
    """
    Get the one-hot encoding of the attribute matrix.

    Parameters
    ----------
    attr: (N, F)
        attribute matrix to get the one-hot encoding from.
    ...
    attr_one_hot_list = []
    num_node_attr = attr.shape[1]
    for f in range(num_node_attr):
        # (N, 2)
        attr_f_one_hot = F.one_hot(attr[:, f], num_classes=2)
