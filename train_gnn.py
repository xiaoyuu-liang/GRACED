...
parser.add_argument('--model_list', nargs='+', required=False, default=['GIN'],
...
for dataset_name in args.dataset_list:
    ...
    datareader = DataReader(data_dir='./data/%s/' % dataset_name.upper(),
...
#                     concat_dropout=args.concat_dropout).to(device)  
        ...
        for fold_id in range(args.n_folds):
            ...
            if args.save_model:
                print('Save model ...')
                create_directory('./checkpoint_classifier/save_model')
                create_directory('./checkpoint_classifier/save_model/' + model_name)
